...
my %fastaHandles = ("path" => "/home/q/qmorris/yulia/hg19"); # human reference genome hg19

###############################################################
my $caller = $ARGV[0]; # caller definition: muse/caveman/dkfz..
my $vcfFile = $ARGV[1];
my $phiFile = $ARGV[2];
my $mutStart = $ARGV[3]; # window size range start point
my $mutEnd = $ARGV[4]; # window size range end point
my $MutOrderFile = $ARGV[5];
###############################################################

# Assigning 96 trinucleotide context into a hash
my $line;
my %counts;
my $countsFile = "/home/q/qmorris/yulia/trinucleotide.txt";
...
while ($line = <VCFFILE>)
{
	if ($line !~ /^#/)
	{
        ...
        # Check on the particular mutation in the vcf file is in the phi window range
        my $phi_in=0;
        for my $i ($mutStart..$mutEnd) {
        	if (defined $phi_array[$i])
        	{
   				if ($phi_array[$i] eq "$chr\t$pos")
				{   			
   					$phi_in++;
   				}
   			}
	...
	my $checkChr=0;
		...
		if ((length($ref) == 1) and ($phi_in == 1) and ($checkChr == 1) and ($checkPass == 1))
		{
...
print "$vcfFile\t";
print mean(@phi_values[($mutStart..min($#phi_values, $mutEnd))]);
my $c=0;
