...
plot_signatures <- function (dd, plot_name, phis = NULL, fitted_data = NULL, mark_max_signature=F, mark_change_points=F, 
                             ...
                             assigns_phylo_nodes = NULL, scale = 1) {
  ...
  decrement <- as.integer(100/ncol(dd))
  for (n in 1:ncol(dd)) {
    col_names <- append(col_names, 100 - decrement*(n-1))
  }
  ...
  g <- ggplot(data = df.m, aes(x = variable, y = value, group = Signatures, color = Signatures)) + 
    geom_line(alpha=alpha) + xlab("Phi") + ylab("Weight") + geom_point(alpha=alpha) + theme_bw() + 
  ...
  if (!is.null(fitted_data))
  {
    fitted_data.m <- data.frame(signatures,fitted_data)
  ...
  if (mark_change_points)
  {
    ...
    g <- g + geom_vline(xintercept = change_points, alpha = 0.3)
  }
  ...
  if (save) {
    suppressWarnings(ggsave(filename = plot_name, width = 12*scale, height=7*scale))
  }
...
plot_signatures_real_scale <- function (dd, plot_name, phis = NULL, fitted_data = NULL, mark_max_signature=F, mark_change_points=F, 
                             change_points=NULL, error_bars = NULL, save=T,
                             assigns_phylo_nodes = NULL, remove_sigs_below = 0) {
  if (!is.null(error_bars) & sum(dim(dd) == dim(error_bars)) != 2) {
    stop("Dimentions of error bar matrix should be the same as dimentions of mixture matrix")
  }
  ...
  g <- ggplot(data = df.m, aes(x = variable, y = value, group = Signatures, color = Signatures)) + 
    geom_line(alpha=alpha,  size = size) + xlab("Phi") + ylab("Weight") + geom_point(alpha=alpha) +
    theme_bw()
  ...
  if (save) {
    suppressWarnings(ggsave(filename = plot_name, width = 12, height=7))
  }
...
add_change_point_distribution <- function(p, changepoints_bootstrap, n_timepoints, ymax, plot_name, save=T, rolsum = 1)
{
  ...
  for (i in 1:length(cp_distr)) { 
    g_prime <- g_prime + geom_rect(xmin = (i-0.2), xmax = (i+0.2), ymin=-0.06, 
                                   ymax = cp_distr[i]/max(cp_distr)*0.05-0.06) + guides(fill=FALSE)
  }
  g_prime <- g_prime + guides(color=guide_legend(override.aes=list(fill=NA))) + ylim(c(-0.05, ymax)) + theme_bw()
