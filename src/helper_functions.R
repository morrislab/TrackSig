...
extract_data_for_example <- function (example, dir_counts, tumortypes) {
  ...
  vcfFile <- gsub(".*/([^/]*)\\.phi\\.txt","\\1", vcfFile) # extract tumor id
  acronym <- toString(tumortypes[tumortypes$ID == vcfFile,]$tumor_type)
  dir_name <- paste0(DIR_RESULTS, acronym, "/", vcfFile, "/")
  suppressWarnings(dir.create(dir_name, recursive = T))
  
  if (is.null(vcfData))
  {
    return(list(vcfFile, vcfData, NULL, NULL, acronym, dir_name))
  }
  ...
  {
    print(paste0("Achting! Example ", example, " phis: ", length(phis), " assigns_phylo_nodes: ",
          length(assigns_phylo_nodes)))
    assigns_phylo_nodes = NULL
  }
  
  return(list(vcfFile, vcfData, phis, assigns_phylo_nodes, acronym, dir_name))
}
...
extract_bootstrap_data_for_example <- function (example, bootstrap_counts) {
  ...
  for (vcfFile in (list.files(paste0(bootstrap_counts, "/", example, "/"))))
  {
    ...
    if (grepl(".*\\.(\\d+)\\.mut_counts.unsorted.txt", vcfFile)) {
       index <- as.numeric(gsub(".*\\.(\\d+)\\.mut_counts.unsorted.txt", "\\1", vcfFile))
       vcfFile <- paste0(bootstrap_counts, "/", example, "/", vcfFile)
       ...
       vcfData.bootstrap.unsorted[[index]] <- vcfData
     }
  }
  
  return(list(vcfData.bootstrap, vcfData.phis, vcfData.bootstrap.unsorted))
}
...
get_examples_group <- function(list_, EXAMPLES_PER_GROUP, group)
{
  if (group == 0)
  {
    ...
    start = (EXAMPLES_PER_GROUP*(group-1))
...
compute_mean_sd_err <- function(mixtures_bootstrap, sig_names, dir_name, descr = "")
{
  ...
  for (i in 1:nrow(mixtures_bootstrap[[1]]))
    for (j in 1:ncol(mixtures_bootstrap[[1]]))
    {
      ...
      mixtures.sd[i,j] <- sd(values, na.rm=T)
      mixtures.err[i,j] <- sd(values, na.rm=T)/sqrt(length(values))
      mixtures.mean[i,j] <- mean(values, na.rm=T)
    }
  rownames(mixtures.mean) <- sig_names
  ...
  return(list(mixtures.mean, mixtures.sd, mixtures.err))
}


get_bootstrap_mixtures <- function(bootstrap_vcfs, bootstrap_phis, alex.t, dir_name, descr)
{
  ...
  {
    print(j)
    print(!file.exists(paste0(dir_name, "changepoints.bootstrap_", j, descr, ".txt")))
